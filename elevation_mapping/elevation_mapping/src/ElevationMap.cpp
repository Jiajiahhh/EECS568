...
bool ElevationMap::add(const PointCloudType::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances, const ros::Time& timestamp,
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i) {
    ...
    if (!isValid) {//in this part we can add semantic part
...
// Semantic Process Part
// Adds semantic information from a point cloud to the elevation map.
bool ElevationMap::addSemantic(const PointCloudType::Ptr pointCloud,  const ros::Time& timestamp,
  ...
  auto& alpha0=rawMap_["alpha0"];
  auto& alpha1=rawMap_["alpha1"];
  auto& alpha2=rawMap_["alpha2"];
  auto& alpha3=rawMap_["alpha3"];
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i) {
    ...
    float tmp = point.b; // 0-3
    if(tmp < 0 || tmp > 3){
      tmp=0;
      }

    auto& a0= alpha0(index(0),index(1));
...
// Updates the elevation map with new variance data.
...
// Perform fusion of elevation map data within specified areas.
  ...
  for (grid_map::SubmapIterator areaIterator(rawMapCopy, topLeftIndex, size); !areaIterator.isPastEnd(); ++areaIterator) {
    ...
    if (i == 0) {
      ...
      fusedMap_.at("color", *areaIterator) = rawMapCopy.at("color", *areaIterator);
       fusedMap_.at("semantic", *areaIterator) = rawMapCopy.at("semantic", *areaIterator);// add semantic fuse process
      continue;
    }
...
// Clears both the raw and fused elevation map data.
...
// Cleans up the elevation map data based on visibility criteria.
  ...
  for (grid_map::GridMapIterator iterator(visibilityCleanupMap_); !iterator.isPastEnd(); ++iterator) {
    ...
    if (timeSinceInitialization - time > scanningDuration_) {
      ...
      const auto& elevation = visibilityCleanupMap_.at("elevation", *iterator);
       const auto& semantic = visibilityCleanupMap_.at("semantic", *iterator);
...
// Moves the elevation map based on a given position.
...
// support for handling update an underlying map 
// This underlying map is then integrated into the elevation map data.
