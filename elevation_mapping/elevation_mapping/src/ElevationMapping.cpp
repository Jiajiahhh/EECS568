...
void ElevationMapping::setupSubscribers() {  // Handle deprecated point_cloud_topic and input_sources configuration.
  ...
  //subscribe to get semantic----------------------------------------------------------------------------------------------------------------------------------
  semanticSub=nodeHandle_.subscribe<sensor_msgs::PointCloud2>("/semantic_pcl",1,std::bind(&ElevationMapping::semanticpointCloudCallback, this, std::placeholders::_1, true, std::ref(sensorProcessor_)));
  //inputSources_.registerCallbacks(*this, make_pair("/semantic_pcl", &ElevationMapping::semanticpointCloudCallback));
}
...
bool ElevationMapping::readParameters() {
  ...
  nodeHandle_.param("point_cloud_topic", pointCloudTopic_, std::string("/camera/depth/color/points"));
  nodeHandle_.param("robot_pose_with_covariance_topic", robotPoseTopic_, std::string("/t265/odom/sample"));
  nodeHandle_.param("track_point_frame_id", trackPointFrameId_, std::string("/base_footprint"));
  ...
  nodeHandle_.param("map_frame_id", mapFrameId_, std::string("map"));//origin is /map
  ...
  if (!ignoreRobotMotionUpdates_) {
    ...
    if (!poseMessage) {
      ...
      if (robotPoseCache_.getOldestTime().toSec() > lastPointCloudUpdateTime_.toSec()) {
        ...
        ROS_ERROR("1Could not get pose information from robot for time %f. Buffer empty?", lastPointCloudUpdateTime_.toSec());
      }
      return;
    }
...
//semantic PCL callback
void ElevationMapping::semanticpointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& pointCloudMsg, bool publishPointCloud,
                                          const SensorProcessorBase::Ptr& sensorProcessor_) {
  ROS_DEBUG("Processing data from: %s", pointCloudMsg->header.frame_id.c_str());
  if (!updatesEnabled_) {
    ROS_WARN_THROTTLE(10, "Updating of elevation map is disabled. (Warning message is throttled, 10s.)");
  ...
  if (!ignoreRobotMotionUpdates_) {
    ...
    if (!poseMessage) {
      ...
      if (robotPoseCache_.getOldestTime().toSec() > lastPointCloudUpdateTime_.toSec()) {
        ...
        ROS_ERROR("2Could not get pose information from robot for time %f. Buffer empty?", lastPointCloudUpdateTime_.toSec());
      }
      return;
    }
    robotPoseCovariance = Eigen::Map<const Eigen::MatrixXd>(poseMessage->pose.covariance.data(), 6, 6);
...
//Add Semantic cloud to Map
  if (!map_.addSemantic(pointCloudProcessed,  lastPointCloudUpdateTime_,
                Eigen::Affine3d(sensorProcessor_->transformationSensorToMap_))) {
    ROS_ERROR("Adding point cloud to elevation map failed.");
    resetMapUpdateTimer();
    return;
