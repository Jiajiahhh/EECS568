...
class ElevationMap {
 public:
  ...
  bool add(const PointCloudType::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances, const ros::Time& timeStamp,
    ...
    bool addSemantic(const PointCloudType::Ptr pointCloud, const ros::Time& timeStamp,
           const Eigen::Affine3d& transformationSensorToMap);
 ...
 private:
  ...
  double semanticNum_;
  std::vector<std::vector<std::vector<int>>> isemantic_;
  std::vector<std::vector<int>> isemanticNum_;

  float calculateSemanticValue(std::vector<int> semantic_value);
};
