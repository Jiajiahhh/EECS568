...
public:
    VisualOdometry(ros::NodeHandle &nh)
    {
        ...
        pose_sub_ = nh.subscribe("/t265/odom/sample", 1000, &VisualOdometry::odomCallback, this);
    }
    ...
    void odomCallback(const nav_msgs::Odometry::ConstPtr &msg)
    {
        ...
        ori_z = orientation.z;
        ori_w = orientation.w;
        av_x = twist.twist.angular.x;
        av_y = twist.twist.angular.y;
        av_z = twist.twist.angular.z;
        la_x = twist.twist.linear.x;
        la_y = twist.twist.linear.y;
        la_z = twist.twist.linear.z;
    ...
    void imageCallback(const sensor_msgs::ImageConstPtr &msg)
    {
        ...
        try
        {
            ...
            if (!current_frame.empty() && !dep.empty())
            {
                ...
                {
                    cout << "doing feature detection" << endl;
                ...
                else
                {
                    ...
                    double cx = 326.8223571777344;
                    double cy = 237.0301513671875;
                    double fx = 608.879638671875;
                    double fy = 607.906494140625;
                    double scale_sum = 0;
                    ...
                    cout<<"finish filtering 2d points"<<endl;
                    ...
                    for (size_t i = 0; i < points2.size(); i++)
                    {
                        ...
                        {
                            curr_d = curr_depth_frame.at<double>(points1[i].y, points1[i].x);
                            prev_d = pre_depth_frame.at<double>(points2[i].y, points2[i].x);
                        }
                        curr_d = curr_d / 1000;
                        prev_d = prev_d / 1000;
                        if ((curr_d >= 0.5 && curr_d <= 100) && (prev_d >= 0.5 && prev_d <= 100))
                        {
                            // transfer 2d to 3d
                            cv::Point3f curr_pt3D;
                            cv::Point3f prev_pt3D;
                            // cout<<curr_d<< " "<<prev_d<<endl;
                            curr_pt3D.x = (points1[i].x - cx) * curr_d / fx;
                            curr_pt3D.y = (points1[i].y - cy) * curr_d / fy;
                            prev_pt3D.x = (points2[i].x - cx) * prev_d / fx;
                            prev_pt3D.y = (points2[i].y - cy) * prev_d / fy;
                            ...
                            curr_pt3D.z = curr_d;
                            prev_pt3D.z = prev_d;
                            validPoints2.push_back(points1[i]);
                    ...
                    cv::Rodrigues(rvec, R);
                    if (abs(tvec.at<double>(0)) > 0.8 || abs(tvec.at<double>(1)) > 0.8 || abs(tvec.at<double>(2)) > 0.8)
                    {
                        // cout << 1 << endl;
                        tvec.at<double>(0) = 0;
                        tvec.at<double>(1) = 0;
                    ...
                    overall_dis_y = -t_f.at<double>(0);
                    overall_dis_z = t_f.at<double>(1);
                    cout << validPoints3.size() << endl;
                    if (validPoints3.size() > 20)
                    {
                        ...
                        cout << "x= " << -overall_dis_x << " y= " << overall_dis_y << " z= " << overall_dis_z  << std::endl
                             << endl;
                        // // cout<<R_f<<endl;
                    }
                    waitKey(1);
...
{
    // initial ros node
    ros::init(argc, argv, "visodo_ros_real");
